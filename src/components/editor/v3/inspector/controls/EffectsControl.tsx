...
export function EffectsControl({
  ...
  return (
    <div className="space-y-4 select-none text-xs">
      {showRadius && (
        <div className={`space-y-2 ${section === 'all' ? 'pt-2 border-t border-border' : ''}`}>
          <div className="flex items-center justify-between">
            <label className="text-[11px] font-semibold text-muted-foreground">Corner radius</label>
            <button
              type="button"
            ...
            >
              {radiusLinked ? <Link2 className="w-3.5 h-3.5 text-primary" /> : <Unlink className="w-3.5 h-3.5" />}
            </button>
          </div>
            ...
            <div className="grid grid-cols-4 gap-1">
              {['topLeft', 'topRight', 'bottomRight', 'bottomLeft'].map((corner) => (
                <input
                  ...
                  className="w-full h-8 text-center rounded-lg bg-muted/50 border border-border text-foreground text-xs"
                />
              ))}
            </div>
          )}
        ...
        <div className={`grid grid-cols-3 gap-2 ${section === 'all' ? 'pt-1' : ''}`}>
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full h-8 px-1.5 rounded-lg bg-muted/50 border border-border text-foreground text-xs"
            >
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-1.5 h-8 px-1.5 rounded-lg bg-muted/50 border border-border">
              <input
                type="color"
              ...
              />
              <span className="text-[10px] font-mono text-muted-foreground truncate">{border.color || '#2A3042'}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {showShadow && (
        <div className={`space-y-2 ${section === 'all' ? 'pt-2 border-t border-border' : ''}`}>
          <label className="text-[11px] font-semibold text-muted-foreground">Box Shadow</label>
          <div className="grid grid-cols-4 gap-1.5">
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full h-7 text-center rounded bg-muted/50 border border-border text-foreground text-xs"
              />
            </div>
            <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 h-7 px-2 rounded bg-muted/50 border border-border text-xs font-mono text-foreground"
              placeholder="#000000"
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            />
            <span className="w-8 text-right font-mono text-[10px] text-primary">
      ...
      {section === 'all' && (
        <div className="space-y-1 pt-2 border-t border-border">
          <div className="flex justify-between items-center text-[11px]">
            <span className="font-semibold text-muted-foreground">Layer opacity</span>
